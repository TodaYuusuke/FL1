...
Particle::Particle(std::unique_ptr<IPlane> plane, bool isStretchBillboard)
{
	// 平面のポインタを受け取る
...
void Particle::Update(const float deltaTime, const float playSpeed)
{
	// 生存時間タイマー終了時
...
Particle& Particle::SetParent(LWP::Object::TransformQuat* parent)
{
	// 親子付け設定
...
Particle& Particle::SetTransform(const LWP::Math::Vector3& scale, const LWP::Math::Quaternion& rotate, const LWP::Math::Vector3& translate)
{
	// 座標設定
...
Particle& Particle::SetAliveTime(const float aliveTime)
{
	// 粒子の生存時間を渡す
...
Particle& Particle::SetVelocity(const LWP::Effect::VelocityData<Vector3>& data)
{
...
Particle& Particle::SetEasing(const LWP::Effect::EasingData<LWP::Math::Vector3>& data)
{
	// 移動用コンポーネントが何も設定されていない場合
	if (moveComponent_ == nullptr) {
		// 初期座標の設定
...
Particle& Particle::SetRotateVelocity(const LWP::Effect::VelocityData<LWP::Math::Vector3>& data)
{
...
Particle& Particle::SetScaleEasing(const LWP::Effect::EasingData<LWP::Math::Vector3>& data, const bool isUnificationScale)
{
...
Particle& Particle::SetColorEasing(const LWP::Effect::EasingData<LWP::Math::Vector4>& data)
{
	// 移動用コンポーネントが何も設定されていない場合
	if (colorComponent_ == nullptr) {
		// 開始色の取得
